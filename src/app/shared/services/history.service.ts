...
export class HistoryService {
  ...
  getTask(query: HistoryQueryRequest) {
    ...
    return this.http.post<HistoryPagedResult<any> | any[]>(
      url,
      this.toQueryPayload(query),
    );
  }
  getTasksByIds(taskIds: string[]) {
    const url = `${this.baseUrl}/History/ByIds`;
    return this.http.post<any[]>(url, { taskIds });
  }
